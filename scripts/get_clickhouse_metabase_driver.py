...
DRIVER_JAR_OUT_PATH = BASE_DIR / "metabase_plugins" / "clickhouse.metabase-driver.jar"

if __name__ == "__main__":
    DRIVER_JAR_OUT_PATH.parent.mkdir(exist_ok=True)
    print(f"Downloading resource: {DRIVER_JAR_URI}")
    DRIVER_JAR_OUT_PATH.write_bytes(urlopen(DRIVER_JAR_URI).read())
    print(f"Download complete. Saved to path: {DRIVER_JAR_OUT_PATH}")
